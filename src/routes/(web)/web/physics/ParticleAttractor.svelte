	...
	useTask((delta) => {
		particles.forEach((particle) => {
			...
			if (mouseDistance < MOUSE_REPEL_RADIUS && mouseDistance > 0) {
				const repelStrength =
					Math.pow(1 - mouseDistance / MOUSE_REPEL_RADIUS, 2) * MOUSE_REPEL_FORCE;
...
{#each particles as particle (particle.id)}
	<T.Group position={[particle.position.x, particle.position.y, particle.position.z]}>
		<RigidBody bind:rigidBody={particle.rigidBody} type="kinematicPosition" lockRotations>
			<Collider shape="ball" args={[PARTICLE_RADIUS]} sensor />
			<T.Mesh
				castShadow
				receiveShadow
...
<T.Group position={[mouseWorldPos.x, mouseWorldPos.y, mouseWorldPos.z]}>
	<T.Mesh rotation.x={-Math.PI / 2}>
		...
		<T.MeshBasicMaterial color={0xffffff} transparent opacity={0.05} side={2} />
	</T.Mesh>
</T.Group>
